...
@implementation HugeHorseModel
-(instancetype)init
{
    self = [super init];
    self.zuoJiModelsArray = [NSMutableArray array];
    return self;
}
...
- (void)setValue:(id)value forKey:(NSString *)key {
    ...
    if ([key isEqualToString:@"zuoJiModelsArray"] && [value isKindOfClass:[NSArray class]])
    {
        NSArray        *array     = value;
    ...
    if ([value isKindOfClass:[NSDictionary class]]) {
        ...
        [self.zuoJiModelsArray addObject:model];
        model.extraTag = key;
