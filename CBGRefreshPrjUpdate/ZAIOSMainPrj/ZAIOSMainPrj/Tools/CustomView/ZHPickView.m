...
-(instancetype)initPickviewWithPlistName:(NSString *)plistName isHaveNavControler:(BOOL)isHaveNavControler title:(NSString *)t {
...
-(instancetype)initDatePickWithDate:(NSDate *)defaulDate datePickerMode:(UIDatePickerMode)datePickerMode isHaveNavControler:(BOOL)isHaveNavControler title:(NSString *)t{
...
-(void)setUpPickView{
    ...
    pickView.backgroundColor= LINE_GAYCOLOR;
...
-(void)setUpDatePickerWithdatePickerMode:(UIDatePickerMode)datePickerMode{
    ...
    datePicker.backgroundColor= LINE_GAYCOLOR;
;
    if (_defaulDate) {
...
//    [leftitem setTitleColor:REDCOLOR forState:UIControlStateNormal];
//    leftitem.frame = CGRectMake(10, 5, 50, 30);
...
//    [leftitem addTarget:self action:@selector(remove) forControlEvents:UIControlEventTouchUpInside];
    ...
    UILabel*title = [[UILabel alloc] initWithFrame:CGRectMake(SCREEN_WIDTH/2 -20, 0, width, height)];
    [toolbar addSubview:title];
    title.font = [UIFont systemFontOfSize:18.0];
    title.textAlignment = NSTextAlignmentCenter;
    title.textColor = [DZUtils  colorWithHex:@"222222"];
    title.text = _desString;
    
    UIButton *rightitem= [UIButton buttonWithType:UIButtonTypeCustom];
    [rightitem setTitle:@"确定" forState:UIControlStateNormal];
    rightitem.titleLabel.font = [UIFont systemFontOfSize:18.0];
    rightitem.frame = CGRectMake(SCREEN_WIDTH -10-50, 5, 50, 30);
    [toolbar addSubview:rightitem];
    [rightitem setTitleColor:REDCOLOR forState:UIControlStateNormal];

    [rightitem addTarget:self action:@selector(doneClick) forControlEvents:UIControlEventTouchUpInside];
    
//    CALayer * layer = leftitem.layer;
//    layer.borderWidth = 1.0;
//    layer.borderColor = [REDCOLOR CGColor];
    
    CALayer * layer = rightitem.layer;
    layer.borderWidth = 1.0;
    layer.borderColor = [REDCOLOR CGColor];
...
-(void)doneClick
{
    ...
    [(UIButton *)_delegate removeFromSuperview];
    [self removeFromSuperview];
}
/**
 *  设置PickView的颜色
