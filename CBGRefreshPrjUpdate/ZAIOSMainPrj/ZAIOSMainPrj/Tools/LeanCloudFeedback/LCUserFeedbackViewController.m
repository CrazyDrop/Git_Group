...
- (void)viewDidDisappear:(BOOL)animated {
    ...
    // 记录最后阅读时，看到了多少条反馈
    [self localSaveUnreadNumber];
}
-(void)localSaveUnreadNumber
{
...
- (void)localBackgroundDataRefresh
{

    if(_refreshControl.isRefreshing) return;
...
//刷新网络，检查未读消息数，消息树大于0，则调用localBackgroundDataRefresh
-(void)localBackgroundUnreadBackNumRefresh
{
    
    __weak typeof(self) weakSelf = self;
    [[LCUserFeedbackAgent sharedInstance] countUnreadFeedbackThreadsWithBlock:^(NSInteger number, NSError *error) {
        if (!error && number>0) {
            [weakSelf localBackgroundDataRefresh];
        }
    }];
}
...
- (void)sendButtonClicked:(id)sender {
    NSString *content = self.inputTextField.text;
    if(!content || [content intValue]<=0) return;
    
    ZALocalStateTotalModel * total = [ZALocalStateTotalModel currentLocalStateModel];
    total.refreshTime = content;
    [total localSave];
...
- (void)keyboardWillShow:(NSNotification *)notification {
    ...
    [UIView animateWithDuration:animationDuration
                     ...
                     animations:^{
                         ...
                         tableViewFrame.size.height = self.view.bounds.size.height - self.tableView.frame.origin.y - keyboardHeight - kInputViewHeight;
...
- (void)keyboardWillHide:(NSNotification *)notification {
    ...
    CGRect tableViewFrame = self.tableView.frame;
    tableViewFrame.size.height = CGRectGetMaxY(self.view.frame) - self.tableView.frame.origin.y  - kInputViewHeight;;
    self.tableView.frame = tableViewFrame;
