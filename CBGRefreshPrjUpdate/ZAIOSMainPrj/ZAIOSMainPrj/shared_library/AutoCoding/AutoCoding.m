    ...
    for (__unsafe_unretained NSString *key in self.codableProperties)
    {
        ...
        if (value)
        {
            ...
            }else if([value isKindOfClass:[NSArray class]])
            {
                ...
                dict[key] = [value dictionaryRepresentation];
            }
        }
    }
    return dict;
...
- (void)setWithCoder:(NSCoder *)aDecoder
{
    ...
    for (NSString *key in properties)
    {
        ...
        if (object)
        {
            ...
            {
                //当有长度方法时，长度不为0
                if(![object respondsToSelector:@selector(length)] || ([object respondsToSelector:@selector(length)] && [object length]!=0))
                {
...
+ (instancetype)objectWithContentsOfData:(NSData *)data
{
    //attempt to deserialise data as a plist
    id object = nil;
    if (data)
