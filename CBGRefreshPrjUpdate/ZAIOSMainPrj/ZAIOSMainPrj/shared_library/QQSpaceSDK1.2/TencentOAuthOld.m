...
@implementation TencentOAuthOld
...
- (TencentRequestOld*)openUrl:(NSString *)url
           ...
           httpMethod:(NSString *)httpMethod
             delegate:(id<TencentRequestOldDelegate>)delegate {
...
- (void)authorizeWithTencentAppAuthInSafari:(BOOL) bInSafari{
	...
	if (bInSafari)
	{
		...
		if ([device respondsToSelector:@selector(isMultitaskingSupported)] && [device isMultitaskingSupported]) {
			...
			if (!didOpenOtherApp) {
				...
				NSString *appUrl = [TencentRequestOld serializeURL:loginDialogURL params:params];
	...
	{
		[_loginDialog release];
		_loginDialog = [[TencentLoginViewOld alloc] initWithURL:loginDialogURL
													  params:params
													delegate:self];
        LYOldQQWebViewController *webviewController = [[LYOldQQWebViewController alloc] init];
        
    
        _loginDialog.dissmis =^ (){
...
//        [nil presentViewController:navController animated:YES completion:^{
//        
//        }];
        webviewController.web = _loginDialog;

        if(self.sessionDelegate && [self.sessionDelegate  respondsToSelector:@selector(showLoginWebViewController:)])
        {
            [self.sessionDelegate showLoginWebViewController:webviewController];
        }
//        [webviewController.view addSubview: _loginDialog];
//        [_loginDialog show];
...
- (BOOL)getUserInfo
{
	...
	APIBaseOld *apiRequest = [[APIBaseOld alloc] initWithTarget:self andSelector:@selector(getUserInfoResponse
                                                                                           :) andHttpMethod:@"GET"];
...
- (BOOL)getListAlbum
{
	...
	NSMutableDictionary * params = [[NSMutableDictionary alloc] init];
	APIBaseOld *apiRequest = [[APIBaseOld alloc] initWithTarget:self andSelector:@selector(getListAlbumResponse:) andHttpMethod:@"GET"];
...
- (BOOL)getListPhotoWithParams:(NSMutableDictionary *)params
{
	...
	APIBaseOld *apiRequest = [[APIBaseOld alloc] initWithTarget:self andSelector:@selector(getListPhotoResponse:) andHttpMethod:@"GET"];
...
- (BOOL)addShareWithParams:(NSMutableDictionary *)params
{
	...
	APIBaseOld *apiRequest = [[APIBaseOld alloc] initWithTarget:self andSelector:@selector(addShareResponse:) andHttpMethod:@"POST"];
...
- (BOOL)uploadPicWithParams:(NSMutableDictionary *)params
{
	...
	APIBaseOld *apiRequest = [[APIBaseOld alloc] initWithTarget:self andSelector:@selector(uploadPicResponse:) andHttpMethod:@"POST"];
...
- (BOOL)addAlbumWithParams:(NSMutableDictionary *)params
{
	...
	APIBaseOld *apiRequest = [[APIBaseOld alloc] initWithTarget:self andSelector:@selector(addAlbumResponse:) andHttpMethod:@"POST"];
...
- (BOOL)addOneBlogWithParams:(NSMutableDictionary *)params
{
	...
	APIBaseOld *apiRequest = [[APIBaseOld alloc] initWithTarget:self andSelector:@selector(addOneBlogResponse:) andHttpMethod:@"POST"];
...
-(BOOL)checkPageFansWithParams:(NSMutableDictionary *)params
{
	...
	APIBaseOld *apiRequest = [[APIBaseOld alloc] initWithTarget:self andSelector:@selector(checkPageFansResponse:) andHttpMethod:@"GET"];
...
- (BOOL)addTopicWithParams:(NSMutableDictionary *)params
{
	...
	APIBaseOld *apiRequest = [[APIBaseOld alloc] initWithTarget:self andSelector:@selector(addTopicResponse:) andHttpMethod:@"POST"];
...
{
	APIBaseOld *apiRequest = [_apiRequests objectForKey:seq];
	if (apiRequest)
		[apiRequest release];
