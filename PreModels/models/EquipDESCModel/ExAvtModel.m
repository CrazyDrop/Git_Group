...
#import "ExAvtModel.h"
#import "ExtraModel.h"
@implementation ExAvtModel
-(instancetype)init
{
    self = [super init];
    self.jinyiModelsArray = [NSMutableArray array];
    return self;
}
...
- (void)setValue:(id)value forKey:(NSString *)key {
    ...
    if ([key isEqualToString:@"jinyiModelsArray"] && [value isKindOfClass:[NSArray class]])
    {
        NSArray        *array     = value;
    ...
    if ([value isKindOfClass:[NSNumber class]]) {
        ...
        [self.jinyiModelsArray addObject:model];
        model.extraTag = key;
