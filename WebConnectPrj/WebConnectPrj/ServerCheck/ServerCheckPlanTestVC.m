...
@interface ServerCheckPlanTestVC ()<UITableViewDataSource,UITableViewDelegate,RefreshCellCopyDelgate>
{
...
@implementation ServerCheckPlanTestVC
...
-(id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if(self){
        ...
        if([serverEdit count] > 0)
        {
            self.totalArr = [serverEdit subarrayWithRange:NSMakeRange(0, 1)];
        }
        
        self.totalArr = @[@33];
        self.inWebRequesting = NO;
        
        dbManager = [[ZALocalModelDBManager alloc] initWithDBExtendString:@"checkTest"];
...
//        if(!dbArr || [dbArr count] == 0)
    ...
    if([models count] > 0)
    {
        ...
        [DZUtils noticeCustomerWithShowText:@"暂无最新数据"];
    }
