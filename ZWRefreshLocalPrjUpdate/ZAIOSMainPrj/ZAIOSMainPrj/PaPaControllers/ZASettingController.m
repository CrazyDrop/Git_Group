...
- (void)viewDidLoad {
    ...
    self.dataArr = @[@"当前利率设置",@"系统购买数据",@"分享给好友",@"刷新时间",@"给我评分",@"用户指南",@"退出登录",@"调研功能"];
...
//    NSArray * array = [[ZALocationLocalModelManager sharedInstance] localLocationsArrayForAppendingDB];
//    [array enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
//        [[ZALocationLocalModelManager sharedInstance] localSaveCurrentLocation:obj];
//    }];
...
//    @[@"安全密码设置",@"紧急联系人设置",@"分享给好友",@"反馈意见",@"给我评分",@"用户指南",@"退出登录",@"调研功能"]
    switch (indexNum) {
        ...
        case 1:
        {
            ZWDetailBuyVC * contact = [[ZWDetailBuyVC alloc] init];
            [[self rootNavigationController] pushViewController:contact animated:YES];
        }
            break;
        case 2:
